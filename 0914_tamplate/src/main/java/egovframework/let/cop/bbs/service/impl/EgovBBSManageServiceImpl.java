...
public class EgovBBSManageServiceImpl extends EgovAbstractServiceImpl implements EgovBBSManageService {
     ...
     * 
     * */
	public Map<String, Object> selectTestBoard(TestBoard testBoard) throws Exception {
		System.out.println("서비스임플 넘어옴");
		ArrayList<TestBoard> result = bbsMngDAO.selectTestBoard(testBoard);
		
		
		Map<String, Object> map = new HashMap<String, Object>();
		int cnt = bbsMngDAO.selecttestBoardCnt(testBoard);
		System.out.println("CNT : "+cnt);
		map.put("selectTest", result);
		map.put("result_Cnt", cnt);
		return map;
	}

	/*TestBoard 글쓰기*/
	public void insertTestBoard(TestBoard testBoard) throws Exception {
		System.out.println("글쓰기 서비스임플");
		int testBoard_idx=bbsMngDAO.insertTestBoard(testBoard);
		System.out.println("insert 반환값 : "+testBoard_idx);
		testBoard.setTestBoard_idx(testBoard_idx);
		insertTestBoardSo(testBoard);
		
	}

	
	private void insertTestBoardSo(TestBoard testBoard) {
		for(int i=0; i<testBoard.getSoNm().length; i++){
			System.out.println("다시 확인 : "+testBoard.getTestBoard_idx());
			testBoard.setTestBoard_check_soNm(testBoard.getSoNm()[i]);
			bbsMngDAO.insertTestBoardSo(testBoard);
		}
		
	}

	/*TestBpard 상세보기, 조회수 증가*/
	@Transactional
	public Map<String, Object> registTestBoard(TestBoard testBoard) throws Exception {
		System.out.println("상세보기,조회수증가 서비스임플");
		Map<String, Object> map = new HashMap<String, Object>();
		bbsMngDAO.uphitsTestBoard(testBoard);
		ArrayList<TestBoard> result = bbsMngDAO.registTestBoard(testBoard);
		System.out.println("ok");
		testBoard.setTestBoard_idx(result.get(0).getTestBoard_idx());
		ArrayList<TestBoard> check = bbsMngDAO.registTestBoardCheck(testBoard);
		//int a = bbsMngDAO.registTestBoardCheck(testBoard);
	...
	public void updateTestBoard(TestBoard testBoard) throws Exception {
		System.out.println("수정 서비스임플");
		//testBoard테이블 수정
		bbsMngDAO.updateTestBoard(testBoard);
		//testBoard_check 데이터 삭제
		bbsMngDAO.deleteTestBoardCheck(testBoard);
		//testBoard_check 데이터 입력
		insertTestBoardSo(testBoard);
	}

	
	/*TestBoard 삭제*/
	public void deleteTestBoard(TestBoard testBoard) throws Exception {
		System.out.println("삭제 서비스임플");
		bbsMngDAO.deleteTestBoard(testBoard);
	}

	public ArrayList<TestBoard> selectTestBoardno(TestBoard testBoard) throws Exception {
		ArrayList<TestBoard> result = bbsMngDAO.selectTestBoardno(testBoard);
		System.out.println("사이즈 : "+result.size());
		//System.out.println("no : "+result.get(1).getTestBoard_no());
		return result;
	}

	public void replyTestBoard(TestBoard testBoard) throws Exception {
		bbsMngDAO.replyUpdTestBoard(testBoard);
		bbsMngDAO.replyInTestBoard(testBoard);
	}

	public void replyInTestBoard(TestBoard testBoard) throws Exception {
		bbsMngDAO.replyInTestBoard(testBoard);
	}
}
